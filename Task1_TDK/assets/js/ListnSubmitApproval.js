$(document).ready(function () {
    ...
    if (AuditNumber != '' && AuditNumber != null) {
        ApprovalList.ViewApproval(AuditNumber)
    }
    else {
        ApprovalList.GetData()
    }
});

function ApprovalList(){}

ApprovalList.ViewApproval = function (AuditNumber)
{
    $('.ListApproval').attr('hidden', 'hidden')
    $('.DetailApproval').removeAttr('hidden')
...
ApprovalList.GetData = function ()
{
    var Search = document.getElementById("txtSearch").value
    var ApprovalViewMobile = $('#ApprovalViewMobile')
    ApprovalViewMobile.empty()
    Loading.BlockPage()
    $.ajax({
        url: "../Approval/GetDataApprovalMobile",
        method: "POST",
        dataType: "JSON",
        data: {
            Search: Search
        ...
        success: function (j) {
            if (j.Result == true) {
                ...
                $.each(j.Data[0], function (i, st) {
                    ...
                    html += '<div  class="col-6 mb-5" style="padding-right: 5px;padding-left: 5px;" onclick="ApprovalList.ViewApproval(\'' + st.AuditNumber + '\')">'
                        ...
                        html += '<div  class="ribbon-target" style="top: 12px;">'
				                    html +='<span class="ribbon-inner bg-warning"></span>Approval'
			                    html +='</div>'
                ...
                ApprovalViewMobile.append(html)
...
SubmitApproval.Back = function()
{
    $('.DetailApproval').attr('hidden', 'hidden')
    $('.ListApproval').removeAttr('hidden')
    ApprovalList.GetData()
}
